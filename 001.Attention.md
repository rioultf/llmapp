...
flowchart LR
    xt(("$$x_t$$"))
    subgraph EtatsInternes["$$\text{États internes}$$"]
        direction LR
        hprev(("$$h_{t-1}$$"))
        ht(("$$h_t$$"))
    end
...
```mermaid
flowchart LR
    subgraph EtatsExternes["$$\text{États externes}$$"]
        direction LR
        sprev["$$s_{t-1}$$"]
        st["$$s_t$$"]
    end
...
```mermaid
flowchart LR
    x["$$x_1,\dots,x_T$$"] --> Enc["$$\mathrm{Encoder}$$"]
    Enc --> c["$$c$$"]
    c --> Dec["$$\mathrm{Decoder}$$"]
    Dec --> y["$$y_t$$"]
```
...
```mermaid
flowchart LR
    subgraph InputsAttn["$$\text{Entrées}$$"]
        direction LR
        xa1(("$$x_1$$"))
        xa2(("$$x_2$$"))
        xaDots["$$\dots$$"]
    ...
    EncAttn["$$\mathrm{Encoder}$$"]

    subgraph StatesAttn["$$\text{États internes}$$"]
        direction LR
        h1(("$$h_1$$"))
        hDots["$$\dots$$"]
        hT(("$$h_T$$"))
    ...
    subgraph AttentionBlock["$$\text{Attention}$$"]
        direction TB
        alpha["$$\alpha_{tj}$$"]
        ctx(("$$c_t$$"))
    end

    DecAttn["$$\mathrm{Decoder}$$"]

    subgraph OutputsAttn["$$\text{Sorties}$$"]
        direction LR
        ya1(("$$y_1$$"))
        yaDots["$$\dots$$"]
        yaT(("$$y_{T'}$$"))
...
```mermaid
flowchart LR
    subgraph Classical["$$\text{Sans attention}$$"]
        direction LR
        xClass(("$$x_1,\dots,x_T$$")) --> EncClass["$$\mathrm{Encoder}$$"]
        EncClass --> cClass(("$$c$$"))
        cClass --> DecClass["$$\mathrm{Decoder}$$"]
        DecClass --> yClass(("$$y_1,\dots,y_{T'}$$"))
        noteClass["$$\text{contexte unique}$$"]
        cClass -.-> noteClass
    end

    subgraph WithAttentionRecap["$$\text{Avec attention}$$"]
        direction LR
        xAttn(("$$x_1,\dots,x_T$$")) --> EncAttnRecap["$$\mathrm{Encoder}$$"]
        EncAttnRecap --> hSeq(("$$h_1,\dots,h_T$$"))
        hSeq --> AttnRecap["$$\text{Attention}$$"]
        AttnRecap --> cAttn(("$$c_t$$"))
        cAttn --> DecAttnRecap["$$\mathrm{Decoder}$$"]
        DecAttnRecap --> yAttn(("$$y_1,\dots,y_{T'}$$"))
        noteAttn["$$\text{contextes spécifiques à }t$$"]
        cAttn -.-> noteAttn
    end

    cClass -. "$$\neq$$" .-> cAttn
    AttnRecap -. "$$\alpha_{tj}$$" .-> cAttn
```
...
```mermaid
flowchart LR
    wijp["$$w'_{ij}$$"] --> soft["$$\mathrm{softmax}$$"]
    soft --> wij["$$w_{ij}$$"]
```
